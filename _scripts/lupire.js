...
description: |
 ...
 See Lumosity's p<a href="https://www.lumosity.com/brain-games/pinball-recall">inball recall</a>.
...
board.style.top=(100+zoom)/2+'vmin';
board.style.left='50%';
board.style.transform='translateX(-50%)';
board.style.transition=speed+'s opacity';
board.style.border='1px solid black';
board.style.padding='1vmin';
board.style.borderRadius='1vmin';
...
function newGame(s,d,z,v){
	...
	game.style.height=zoom+'vmin';
	game.style.width=zoom+'vmin';
	game.style.borderRadius=1/2*unit+'vmin';
...
function newBoard(){
	...
	for(var i=0;i<span;i++){
		...
		for(var j=0;j<span;j++){
			...
			grid.style.height=unit+'vmin';
			grid.style.width=unit+'vmin';
			grid.style.position='absolute';
			grid.style.top=i*unit+'vmin';
			grid.style.left=j*unit+'vmin';
			grid.style.boxSizing='border-box';
			grid.style.boxShadow='inset 0 0 3vmin '+game.style.backgroundColor;
			...
			box.style.top=i*unit+'vmin';
			box.style.left=j*unit+'vmin';
			...
			if((i==0 && j==0) || (i==0 && j==span-1) || (i==span-1 && j==span-1) || (i==span-1 && j==0)){
				...
				if(i==0 && j==0){
					...
					exit1.style.height=1/2*unit+'vmin';
					exit1.style.width=1/10*unit+'vmin';
					exit1.style.borderRadius=100/3+'%';
					exit1.style.position='absolute';
					exit1.style.top=1/4*unit+'vmin';
					exit1.style.left=9/20*unit+'vmin';
			...
			if(b==full || b==empty){
				...
				box.style.height=unit+'vmin';
				box.style.width=unit+'vmin';
			}
			else if(b==bslash || b==slash){
				box.style.background=color;
				box.style.left=(j+2/5)*unit+'vmin';
				box.style.height=unit+'vmin';
				box.style.width=1/5*unit+'vmin';
				box.style.transform='rotate('+[45,-45][b.img]+'deg)';
			}
			else if(b==cornertr || b==cornerbr || b==cornerbl || b==cornertl){
				box.style.height=unit+'vmin';
				box.style.width=unit+'vmin';
				box.style.borderRight=1/2*unit+'vmin solid '+(b.img[0] ? color : 'transparent');
				box.style.borderTop=1/2*unit+'vmin solid '+(b.img[1] ? color : 'transparent');
				box.style.borderLeft=1/2*unit+'vmin solid '+(b.img[2] ? color : 'transparent');
				box.style.borderBottom=1/2*unit+'vmin solid '+(b.img[3] ? color : 'transparent');
			}
			else if(b==vpipe || b==hpipe){
				box.style.height=unit+'vmin';
				box.style.width=unit+'vmin';
				box.style.borderRight=1/5*unit+'vmin solid '+(b.img[0] ? color : 'transparent');
				box.style.borderTop=1/5*unit+'vmin solid '+(b.img[1] ? color : 'transparent');
				box.style.borderLeft=1/5*unit+'vmin solid '+(b.img[2] ? color : 'transparent');
				box.style.borderBottom=1/5*unit+'vmin solid '+(b.img[3] ? color : 'transparent');				
			}
			else if(b==ccw || b==cw){
				box.style.background=color;
				box.style.left=(j+2/5)*unit+'vmin';
...
function newBall(x,y){
	...
	ball.style.top=x*unit+'vmin';
	ball.style.left=y*unit+'vmin';
	ball.style.width=2/5*unit+'vmin';
	ball.style.height=2/5*unit+'vmin';
	ball.style.borderRadius='50%';
	ball.style.margin=3/10*unit+'vmin';
...
function move(h,k,x,y,z){
	...
	x+=next[to][0];
	y+=next[to][1];
	ball.style.top=unit*x+'vmin';
	ball.style.left=unit*y+'vmin';
	if(x!=0 && x!=span-1 && y!=0 && y!=span-1){
