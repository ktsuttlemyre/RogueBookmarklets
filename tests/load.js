...
function ajax(url,callback){
    ...
    function interpret(yml,args){
        args=args||{}
        return run(RogueBM.stringFormat(yml,args))
    }
    ...
    function run(rogueYML,callback){ //interpret input/yaml
        ...
        for(var index=0,l=commands.length;index<l;index++){
            ...
            if(scriptEntry.yml){
                ajax(scriptEntry.yml,function(err,yml){ 
                    //TODO maybe cache the yml?
                    //RogueBM.cacheCommand(scriptEntry.yml,)
                    RogueRunner.interpret(yml,args)
                })
                return 
            }
